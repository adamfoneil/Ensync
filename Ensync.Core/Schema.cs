...
public class Schema
{
	...
	public async Task<IEnumerable<ScriptAction>> CompareAsync(Schema targetSchema, SqlScriptBuilder scriptBuilder, bool debug = false)
	{
		...
		return results.OrderBy(ActionOrder);

		static int ActionOrder(ScriptAction action)
		{
			// calculated column alters should go towards the end
			if (action.Action == ScriptActionType.Alter && action.Object is Column col && col.IsCalculated) return 4;
	...
	private static bool IsAltered((Column Source, Column Target) columnPair) =>
		columnPair.Source.IsAltered(columnPair.Target).Result;
